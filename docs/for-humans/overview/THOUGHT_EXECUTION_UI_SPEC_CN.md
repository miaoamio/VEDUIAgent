...
- “流程推进行”包含 Thought 与 Spec Hint：当最后一行是“读取表格规范”等 Spec Hint 时，也需要能显示呼吸点（避免看起来卡住）。
...
原因：
- UI 呼吸点逻辑曾只把 `thought` 视为“过程行”，而“读取表格规范”会被解析为 `spec_hint`，导致最后一行是 `spec_hint` 时无法挂载呼吸点。

解决方式：
- 呼吸点应绑定到“过程行”，过程行包含 `thought` 与 `spec_hint`。
- 参考实现：AI 消息渲染中 `hasProcessItem` / `shouldShowLineDots` 逻辑（[App.tsx:L8709-L8846](file:///Users/bytedance/VEDUIAgent/src/App.tsx#L8709-L8846)）。
...
- 参考实现：
  - 缺少 action 的恢复策略（[App.tsx:L6433-L6440](file:///Users/bytedance/VEDUIAgent/src/App.tsx#L6433-L6440)）
  - `read_specs` 分支末尾显式 `continue`（[App.tsx:L6691-L6722](file:///Users/bytedance/VEDUIAgent/src/App.tsx#L6691-L6722)）
