...
- 当上述机器输出默认隐藏时：
  - 不直接把原始 streaming/raw/json 文本展示给用户；
  - UI 必须保证仍有一条可见状态承接流程推进，优先复用最后一个可见过程状态；
  - 若当前没有任何可见过程状态，则显示兜底 `思考中...`。
- 该兜底状态应在**原位承接/原位替换**，而不是通过额外插入临时状态行来补空白。
...
- 文字：
  ...
  - 左对齐，不省略换行（允许多行）
- 对于“创建/设置任务计划”类 thought：
  - 对话流只展示“创建什么任务计划”这一层摘要；
  - 不在 thought 文案中展示“拆成哪些子任务 / 依赖如何配置”等细节；
  - 任务拆分细节留给任务面板承接。
...
- 对齐：
  ...
  - 文字整体左对齐，避免抖动
- 稳定性：
  - Thought 与 Spec Hint 行应使用统一的图标占位宽度与最小高度，减少 `读取规范 ↔ 创建/计划` 切换时的轻微 reflow。
  - `Spec Hint` 布局应与 `Thought` 一致，避免 `fit-content` 等布局方式在临界宽度时造成抖动。
- 最大宽度：
...
- 呼吸点 `...` 应作为**独立尾部占位**渲染：
  - 不参与正文换行计算；
  - 不因 dots 的出现/消失把正文挤到下一行；
  - 允许保留动画，但应通过独立尾缀而不是正文字符拼接实现。
...
- 当最后一条 AI 消息已有可见过程状态时，隐藏的机器输出应优先**原位替换/复用该状态位**，而不是再额外插入一条“思考中”行。
- 首条 AI 消息不应再依赖整条消息容器的位移动画来表达“有新数据流入”；状态推进应主要依赖 thought/spec_hint 与呼吸点本身。
...
- 或者：下一阶段真正写入的是隐藏的 `[Streaming]: ...` / `[JSON]: ...` / `[Raw]: ...`，但当前消息没有任何可见过程状态承接，于是用户会看到“读取规范...”之后短暂空白。
...
- 当隐藏机器输出默认不直显时，应复用最后一个可见过程状态；若当前没有任何可见过程状态，则回退为 `思考中...`。
...
- 或者 `...` 直接拼在正文后，参与换行计算，导致短文案也会因为 dots 出现/消失而轻微跳动。
- 或者整条最后一条 AI 消息仍挂有额外的位移动效，造成首轮消息块整体上浮。

解决方式：
- 保留历史状态行上的 `...`，但只允许它稳定挂在**最后一个可见过程状态**上。
- 隐藏机器流输出时，不新增用户无感知价值的调试块；只补一条简洁、可见的过程文案承接状态。
- `...` 应改为独立尾部占位，不参与正文换行。
- 不再使用整条 AI 消息容器的 `translateY(4px)` pulse 动画制造“有新数据流入”的错觉。
...
- 不再对“最后一条 AI 消息整体”施加 `translateY(4px) → 0` 的 pulse 位移动画。
- 状态推进主要依赖：
  - 可见的 Thought / Spec Hint；
  - 最后一条过程状态上的呼吸点；
  - 必要时的 `思考中...` 兜底。
- 若后续需要新增动效，应优先考虑不引发布局位移的 opacity / color 类动效，避免首条消息与空态切换叠加造成抖动。
