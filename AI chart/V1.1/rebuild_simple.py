...
var drawChart = async function(data, options) {
  ...
  if (frame.layoutMode !== "NONE") {
      ...
      chartContainer.resize(frame.width - 32, frame.height - 32); // 16px padding
      chartContainer.x = 16;
      chartContainer.y = 16;
  ...
  // If frame has 16px padding, then 0 spacer means 16px visual margin.
   ...
   var xAxisHeight = 16; 
   var topMargin = 8; // Re-added 8px to ensure visual gap between Title and Top Label (which sits at y=0 but extends up)
   // Actually, since Label is centered on y=0 (Grid Line), its top is at -5px (fontSize 10).
   // Title Bottom -> Spacing(8px) -> PlotFrame Top.
   // Label Top is at PlotFrame Top - 5px.
   // Visual Gap = 8 - 5 = 3px. Too small.
   // We need Visual Gap = 8px.
   // So PlotFrame Top needs to be 8 + 5 = 13px below Title Bottom.
   // Spacing is 8px. We need 5px more.
   // So topMargin = 6px (round up)?
   
   // Wait, if topMargin is inside PlotFrame (no, it affects plotH), then y=0 is relative to PlotFrame.
   // If we add a Spacer frame between Title and PlotFrame?
   // Or just set itemSpacing to 14?
   // But itemSpacing affects spacing between ChartBody and Title.
   // ChartBody contains PlotFrame.
   // ChartBody has no padding.
   
   // Let's set itemSpacing to 14px? No, that affects other items too (like Legend if it was there, but Legend is usually at bottom).
   // Actually, Title is item 1. ChartBody is item 2.
   // Inside ChartBody: TopSpacer (if any), PlotFrame, XAxisFrame.
   // If we add TopSpacer of 6px inside ChartBody:
   // Title -> 8px -> ChartBody Top -> 6px Spacer -> PlotFrame Top.
   // Total distance = 14px.
   // Label Top is at PlotFrame Top - 5px.
   // Visual Gap = 14 - 5 = 9px. Close to 8px.
   
   // Let's try adding a 6px spacer.
   
   var topSpacerHeight = 6;
   // Ensure it's not too small if no title
   if (!options.title || options.title.length === 0) topSpacerHeight = 0; 
  ...
  // Draw Grid & Y-Axis Labels
    ...
    for (var i = 0; i <= gridSteps; i++) {
        ...
        var labelText = formatYValue(value);
  ...
  if (showXLabels) {
      ...
      if (isCategorical) {
          ...
          // If strict modulo search failed (e.g. prime number of intervals), try relaxed search
          if (bestS === -1) {
               // Try to find ANY step that respects width constraint
               for (var s = 1; s < count; s++) {
                  var numLabels = Math.floor((count - 1) / s) + 1;
                  var currentStepX = stepX * s;
                  var limitW = (currentStepX - minGap);
                  
                  // Check against actual label width if available
                  // We want label to be "fully visible"
                  ...
                  // If limitW < maxLW, labels might be truncated.
                  // But we don't want to be TOO strict (e.g. one very long label ruins everything).
                  // Let's require at least 80% of max width, or just a hard minimum.
                  // User said "reduce label count to let label display fully".
                  // So we should enforce limitW >= maxLW (roughly).
                  
                  if (maxLW > 0) {
                      if (limitW < maxLW) continue; // Skip if space is less than max label width
                  } else {
                      if (limitW < 36) continue; // Fallback to 36px (was 20px, requested increase from 16px to 36px)
                  }
                  
                  var currentDiff = Math.abs(numLabels - targetCount);
                  var bestDiff = (bestS === -1) ? Infinity : Math.abs(bestCount - targetCount);
          ...
          // If still no valid S found (e.g. huge number of points, tiny width), force a large step
          if (bestS === -1) {
              // Fallback: Pick a step that ensures at least 56px per label (adjusted to match targetInterval)
              // User reported "too many labels, all truncated with ...". 
              // Increasing safeStep ensures fewer labels.
              var safeStep = Math.ceil(56 / stepX);
              
              // Also consider Max Label Width in Fallback
              var maxLW = 0;
              if (labelWidths && labelWidths.length > 0) {
                  maxLW = Math.max(...labelWidths);
              }
              if (maxLW > 56) {
                  // If labels are naturally long, increase requirement to ensure visibility
                  safeStep = Math.ceil((maxLW + minGap) / stepX);
              }
              
              if (safeStep < 1) safeStep = 1;
              if (safeStep >= count) safeStep = count - 1;
              
              bestS = safeStep;
              bestCount = Math.floor((count - 1) / bestS) + 1;
          ...
          if (bestS !== -1) {
              ...
              // Ensure last index is included if not exact match (for relaxed mode)
              if (indices[indices.length - 1] !== count - 1) {
                  // If last is too close to current last, replace it?
                  // Or just append?
                  // Appending might overlap. Replacing is safer for layout.
              ...
              // Ensure they don't overlap by limiting width to half
  ...
  // Draw Abnormal Intervals (MarkArea)
  if (options.type === 'abnormal' && options.abnormalIntervals && options.abnormalIntervals.length > 0) {
  ...
  if (options.type === 'threshold' && options.thresholds && options.thresholds.length > 0) {
      options.thresholds.forEach(function(t) {
  ...
  // Legend
  if (showLegend) {
      // 1. Calculate Content and Pagination Logic First
      
      // Calculate widths to determine if pagination is needed
      var tempText = figma.createText();
      tempText.fontSize = labelFontSize;
      tempText.fontName = { family: "Inter", style: "Regular" };
      try { tempText.fontName = { family: "PingFang SC", style: "Regular" }; } catch(e) {}
      ...
      data.datasets.forEach((ds, i) => {
          ...
          var w = 18 + 8 + tempText.width; // Icon(18) + Gap(8) + Text
          itemWidths.push(w);
          totalLegendW += w;
      });
      ...
      var pagerW = 60; // Approx width for < 1/N >
      ...
      if (totalLegendW > drawW) {
          ...
          for (var i = 0; i < itemWidths.length; i++) {
              var itemW = itemWidths[i];
              if (currentW + itemW <= availW) {
                  currentW += itemW + gap;
                  visibleCount++;
              } else {
                  break;
      ...
      // 2. Create Main Legend Container (Horizontal, Space Between)
      ...
      legendContainer.primaryAxisAlignItems = "SPACE_BETWEEN"; // Key: Items Left, Pager Right
      legendContainer.counterAxisAlignItems = "CENTER";
      legendContainer.itemSpacing = 0; 
      legendContainer.fills = [];
      
      // 3. Create Items Wrapper (Left Side)
      ...
      for (var i = 0; i < visibleCount; i++) {
        ...
        // Skip if highlightIndices is present AND this index is NOT in it (Prioritize Highlighted)
        // Wait, logic is: "优先展示高亮曲线对应的图例" (Prioritize displaying legend for highlighted curves)
        // This means we should SORT the datasets or pick visible ones based on highlight status.
        // Current logic just takes first N.
        
        // Re-ordering logic:
        // We need to re-sort `data.datasets` (or a copy) so highlighted ones come first?
        // But re-sorting changes legend order relative to line order?
        // Usually legend order should match line order or be logical.
        // If we just want to ensure highlighted ones are VISIBLE, we should pick them first.
        // Let's create a sorted index list.
      }
      
      // Let's refactor the item selection logic above
      var sortedIndices = data.datasets.map((_, idx) => idx);
      
      if (options.highlightIndices && options.highlightIndices.length > 0) {
          // Sort: Highlighted first, then others
          sortedIndices.sort((a, b) => {
              var aH = options.highlightIndices.indexOf(a) !== -1;
              var bH = options.highlightIndices.indexOf(b) !== -1;
              if (aH && !bH) return -1;
              if (!aH && bH) return 1;
              return a - b; // Keep original order otherwise
          });
      }
      
      // Now recalculate widths based on sorted order
      itemWidths = [];
      totalLegendW = 0;
      
      sortedIndices.forEach(idx => {
          var ds = data.datasets[idx];
          var cleanName = (ds.name || `Series ${idx+1}`).replace(/[\u200B-\u200D\uFEFF]/g, '');
          tempText.characters = cleanName;
      ...
      // Check Pager again with sorted list
      showPager = false;
      visibleCount = data.datasets.length;
      if (totalLegendW > drawW) {
          showPager = true;
          var availW = drawW - pagerW - gap; 
          var currentW = 0;
      ...
      // Render Loop using sortedIndices
      for (var k = 0; k < visibleCount; k++) {
        var originalIndex = sortedIndices[k];
        var ds = data.datasets[originalIndex];
        ...
        line.resize(13, 2);
        line.cornerRadius = 1; 
        line.fills = [{ type: 'SOLID', color: hexToRgb(ds.color) }];
        line.x = (18 - 13) / 2; 
        line.y = (18 - 2) / 2;  
        iconFrame.appendChild(line);
        
        // Restore dot (ellipse) as per design requirement
        var dot = figma.createEllipse();
        dot.resize(8, 8); 
        dot.fills = [{ type: 'SOLID', color: hexToRgb(ds.color) }];
        dot.x = (18 - 8) / 2; 
        dot.y = (18 - 8) / 2; 
      ...
      if (showPager) {
          ...
          pager.itemSpacing = 4;
          pager.fills = [];
          pager.resize(pagerW, 20); // Fixed Height 20px
      ...
      // Add explicit spacing (margin) between chart and legend?
      // chartContainer itemSpacing is 8.
      // So chartBody -> 8px -> legendContainer.
      // This matches requirement "图表下方8px处".
  }
