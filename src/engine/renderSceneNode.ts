...
async function createFigmaNode(
  ...
  if (sceneNode.componentId === "tag") {
    ...
    const componentKey =
      (componentTokenFromProps ? resolveComponentKeyFromToken(componentTokenFromProps) : "") ||
  ...
  if (nodeType === "INSTANCE") {
    ...
    const componentKeyFromDefinitionToken = componentTokenFromDefinition
      ? resolveComponentKeyFromToken(componentTokenFromDefinition)
      : "";
    ...
    if (!componentKey) {
      throw new Error(
        `Component '${sceneNode.componentId}' requires props.componentToken or props.componentKey for instance rendering`
      );
    }
    ...
    try {
      const instance = await createFigmaComponentInstanceFromRef({
        componentKey,
        componentToken: componentTokenFromProps || componentTokenFromDefinition,
        fallbackName,
