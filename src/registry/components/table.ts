  ...
  "table": {
    ...
    "prompts": {
      ...
      "examples": [
        ...
        "多级表头 + 合并: { \"headerRows\": [[\"业务\", \"VRegion\", \"在线计算\", \"\", \"\", \"\", \"\", \"\"], [\"\", \"\", \"TCE(Cores)\", \"TCE(内存GB)\", \"FaaS(Cores)\", \"FaaS(内存GB)\", \"CronJob(Cores)\", \"CronJob(内存GB)\"]], \"rows\": [[\"推荐\", \"China-East\", \"1,305.25万\", \"1,305.25万\", \"1,305\", \"4,046\", \"11\", \"6\"], [\"\", \"China-North\", \"1,305.25万\", \"8,905.25万\", \"20.38万\", \"42.25万\", \"4,400\", \"6871.65\"]], \"merges\": [{ \"section\": \"header\", \"row\": 0, \"col\": 0, \"rowspan\": 2, \"colspan\": 1 }, { \"section\": \"header\", \"row\": 0, \"col\": 1, \"rowspan\": 2, \"colspan\": 1 }, { \"section\": \"header\", \"row\": 0, \"col\": 2, \"rowspan\": 1, \"colspan\": 6 }, { \"section\": \"body\", \"row\": 0, \"col\": 0, \"rowspan\": 2, \"colspan\": 1 }], \"columnTypes\": [\"Text\", \"Text\", \"Text\", \"Text\", \"Text\", \"Text\", \"Text\", \"Text\"] }",
        "订单合计行（Body Colspan）: { \"headers\": [\"订单号\", \"商品名\", \"单价\", \"数量\", \"总价\"], \"rows\": [[\"NO001\", \"商品A\", \"￥199\", \"2\", \"￥398\"], [\"NO002\", \"商品B\", \"￥299\", \"1\", \"￥299\"], [\"NO003\", \"商品C\", \"￥99\", \"5\", \"￥495\"], [\"NO004\", \"商品D\", \"￥49\", \"3\", \"￥147\"], [\"NO005\", \"商品E\", \"￥159\", \"2\", \"￥318\"], [\"合计\", \"\", \"\", \"\", \"￥1,657\"]], \"columnTypes\": [\"Text\", \"Text\", \"Text\", \"Text\", \"Text\"], \"merges\": [{ \"section\": \"body\", \"row\": 5, \"col\": 0, \"rowspan\": 1, \"colspan\": 4 }] }"
      ]
    },
    "renderNotes": {
      "actionHint": "新建表格必须使用 draw_table 紧凑 payload。单层表头优先用 { headers, rows, columnTypes?, columnWidths? }；多级表头或合并单元格优先用 { headerRows, rows, merges?, columnTypes?, columnWidths? }。避免输出 apply_scene 的表格子树。",
      "paramRules": [
        "若消息里出现 \"表格结构(JSON)\"，优先使用其中的 headers/rows 生成表格，不要忽略已上传表格。",
        "若用户描述了多级表头、表头分组、跨行/跨列合并，优先输出 headerRows 和 merges，不要强行压平为单层 headers。",
        "若用户目标是'根据上传图片/表格生成'，直接 draw_tabl / draw_form / create_node 落地（表格/表单无需读取 spec）。",
        "rows 必须提供 2–3 行具体数据；普通表格每个单元格都应填入贴合业务场景的真实内容（如人名、日期、金额、状态词），严禁输出空数组。若使用 merges，被合并覆盖的占位单元格允许为空字符串，不要自动补 '-' 或其他占位符。"
      ],
      "commonErrors": [
        "新建表格时不要使用 apply_scene，直接 draw_table/draw_tabl。",
        "rows 为空数组 [] 会导致所有单元格显示占位符（Cell/Tag/User），必须提供真实数据行。",
        "声明 merges 时不要遗漏 headerRows；若只传单层 headers，就不要输出多级表头 merge。"
      ]
    },
    "params": {
      "size": {
